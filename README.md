...
事件归属保持清晰：WebView 里的页面、点击、输入、提交和 Web 路由仍上报 `platform: "web"`；壳层启动、前后台、原生页面和 deeplink 仍上报 `platform: "ios"`、`"macos"` 或 `"android"`。Web Auto Capture 把 `sourceDetails.framework` 统一解释为运行容器：有效的具体 `data-tracemind-framework` 值优先；`hybrid` 只是通用兜底，脚本会按 Capacitor、Electron、Tauri、Cordova、PWA 的高置信度宿主信号依次识别，无法识别时保留 `hybrid`，普通页面则为 `browser`。Electron/Tauri 未向页面暴露官方宿主信号时应显式配置具体值；不使用 UA 或内部变量猜测。浏览器品牌/版本继续由 `deviceInfo.userAgent` 表达，原生壳层的 metadata 不变。
...
TraceMind 会记录采集来源并在控制台展示采集来源统计。这里的 `sourceType/sourceKey` 用于治理写入项目 key 的 App 或 SDK，不等同于用户跳转进来的流量来源。Web 来源会归一化为 `sourceType: "web"` 和 hostname `sourceKey`，新事件的 `sourceDetails.framework` 表示运行容器：普通页面为 `browser`，高置信度宿主可为 `capacitor`、`electron`、`tauri`、`cordova` 或 `pwa`，显式具体值保持兼容；iOS 使用 bundle id；macOS 使用 bundle id 并上报 `sourceType: "macos"`；Android 使用 package name；React Native 复用对应原生来源并额外标记 `deviceInfo.framework: "react_native"`；混合应用不新增 `hybrid` 事件平台，WebView 保持 Web 来源，原生壳层保持对应 Native 来源；小程序使用 `sourceType: "mini_program"`，`sourceKey` 优先使用 appId，`sourceDetails.provider` 标记微信、支付宝、抖音或钉钉；浏览器插件使用 `sourceType: "browser_extension"`，`sourceKey` 优先使用 extension id，`sourceDetails` 只白名单保存 `browser`、`manifestVersion`、`runtimeContext`、`sdkVersion` 和 `sdkContentHash`；MCP server 使用 `sourceType: "mcp_server"`；普通后端服务使用 `sourceType: "server_app"`；Agent Skill hook 使用 `sourceType: "agent_skill"`。SDK runtime 会安全上报 `sourceDetails.sdkVersion` 和 `sourceDetails.sdkContentHash`；`project_health` 可据此提示 SDK 过旧或版本未知，并生成给 coding agent 的升级动作。SDK 本地源码接入必须按 `capture_setup.latestSdk.sourceRef` 获取源码；发布版使用不可变 `tracemind-release-<version>` tag，避免从浮动 `main` 安装到不匹配内容。开发者发现不是自己项目的来源后，可以在控制台屏蔽该来源。屏蔽规则只匹配精确的 `sourceType + sourceKey`，不会跨来源类型推断关联环境；例如 `web:localhost` 不会同时屏蔽 `server_app:yezi2-server`，后者需要在来源列表中单独屏蔽。屏蔽从生效时刻开始阻止该来源新增事件、Presence、用户反馈、投递诊断和小时健康汇总，采集接口仍返回正常 ok；已经存储的历史证据不会删除或在查询时隐藏，已屏蔽来源也会继续显示，方便解除屏蔽。批量请求若包含多个精确来源，业务记录仍逐条处理，但无法安全拆分的 batch-level 投递统计不会写入投递诊断或小时健康汇总。
