...
通过 `tracemind.capture_setup({ platform: "ios" })` 获取真实安装步骤：从 TraceMind GitHub 源码仓库复制 `sdk/ios` 到客户项目的 `vendor/TraceMind`，再作为本地 Swift Package 加入应用。随后在 App 入口初始化：
...
通过 `tracemind.capture_setup({ platform: "android" })` 复制 `sdk/android` 到 `vendor/tracemind-android`，在 Gradle 中加入本地 `:tracemind` module，随后在 `Application.onCreate()` 初始化：
...
通过 `tracemind.capture_setup({ platform: "react_native" })` 复制 `sdk/react-native` 到 `vendor/tracemind/react-native`，按当前项目 lockfile 选择 npm/pnpm/yarn 的本地 file dependency 命令，并完成 iOS/Android 原生安装步骤：
...
小程序是独立运行时平台，不复用 Web `capture.js`，因为微信、支付宝、抖音、钉钉小程序没有 DOM/script 注入环境。V1 使用一套通用 `sdk/mini-program`，通过 `provider` 区分宿主，不为每个平台复制 SDK。当前通过 `tracemind.capture_setup` 返回的 GitHub 源码 vendoring 命令安装本地 `@tracemind/mini-program` file dependency。
...
浏览器插件是独立运行时平台，不复用 Web `capture.js`。V1 使用一套通用 `sdk/browser-extension`，支持 Chrome、Edge 和 Firefox 的插件自有界面：popup、options、sidebar、devtools page，以及 background/service worker 里的手动业务事件。当前通过 `tracemind.capture_setup` 返回的 GitHub 源码 vendoring 命令安装本地 `@tracemind/browser-extension` file dependency。
...
Node MCP 使用本地 `@tracemind/mcp-node` file dependency；Python MCP 复制 `tracemind_mcp` 包目录并把 vendored 路径加入 `PYTHONPATH` 或项目 packaging source path。具体命令以 `tracemind.capture_setup` 返回值为准。
...
普通后端服务第一版先提供手动埋点能力，不做 request Auto Capture，避免把每个 HTTP 请求、日志或中间件细节变成低价值噪音。coding agent 应通过 `tracemind.capture_setup({ platform: "server_node" })`、`server_python` 或 `server_http` 获取当前项目的一行初始化、payload 模板和隐私规则；Node/Python SDK 当前也走 GitHub 源码 vendoring，`server_http` 不需要 SDK 安装。服务端 SDK 不参与“用户在线时长”统计；服务实例存活监控属于后续独立能力，不能和用户端在线时长混用。
