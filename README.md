...
- `tracemind_raw_behaviors` 是原始行为日志，按 `occurredAt` 保留 10 天。超过窗口后，`tracemind.query_raw_behaviors` 查不到明细不代表数据丢失，应先用 `tracemind.query_events`、`tracemind.summary` 和 `tracemind.project_health` 排查。
