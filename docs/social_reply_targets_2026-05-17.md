...
- 只回复对方正在发布产品、找用户、求反馈、或明确表达“不知道用户卡在哪”的帖子。
- 回复先围绕对方产品的问题，不直接硬广。
- 首轮尽量不放链接，用“我们在做 TraceMind”建立上下文；对方感兴趣后再给链接或私信。
- 每天控制在 5-8 条高相关回复，避免平台判定为批量推广。
- 对长期价值用户优先私信或长回复，不在无关讨论里强推。

## 配图素材

已从 TraceMind 公共介绍页裁出 3 张适合社交传播的截图：
...
执行记录：尝试在 X 主帖下补发带图回复时，Chrome 扩展文件上传被拦截，未发出补图回复。需要在 Chrome 扩展详情中开启 `Allow access to file URLs` 后再上传本地截图。
...
原因：这 5 条的上下文最自然，既不是硬广，也能准确表达 TraceMind 的优势：AI 自动埋点、AI 分析数据、AI 迭代闭环。
