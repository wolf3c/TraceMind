...
查询语义事件。LLM 做产品分析时应该优先使用这个工具，因为它返回的是已经抽取过的业务可读事件。语义事件明细保留 10 天；更早的历史分析使用 `tracemind.project_health`、小时报告和日报。
...
查询原始行为日志。只有在需要复核语义事件背后的原始采集数据时使用。原始行为明细当前保留 10 天；超过明细窗口查不到原始记录时，应先使用 `tracemind.project_health`、日报和小时报告，不要直接判断为数据丢失。
...
```json
{
  ...
  "dataRetention": {
    "detailWindows": [
      ...
      { "dataSet": "presence_sessions", "retentionDays": 10 },
      { "dataSet": "raw_behaviors", "retentionDays": 10 },
      { "dataSet": "semantic_events", "retentionDays": 10 }
    ],
    "retainedSummaries": [
...
返回当前项目的 Auto Capture 公开项目 key、指定平台的一行接入代码、结构化安装指南和安全说明。Coding agent 应先调用它获取当前项目 key；Web 项目验证返回的 `captureScriptUrl`、`data-tracemind-token` 和 `sourceDetails.scriptReleaseId`。Web `sourceModel` 还说明 `framework` 是运行容器：普通页面默认 `browser`，具体显式值优先，Hybrid snippet 保留 `data-tracemind-framework="hybrid"` 并在高置信度宿主信号存在时解析为 Capacitor、Electron、Tauri、Cordova 或 PWA；Electron/Tauri 隐藏官方全局时需显式配置。该说明不新增返回字段或改变 Web/Hybrid 的 platform/sourceType。生产默认 `captureScriptUrl` 来自 Cloudflare Pages `https://tracemind-capture.pages.dev/capture.js`，但 `captureApiUrl`、`presenceApiUrl` 和 `userFeedbackApiUrl` 仍是 Galaxy API。Native 项目使用返回的 SDK 安装步骤和初始化代码，小程序使用通用 SDK 并通过 `provider` 区分宿主，浏览器插件使用通用 WebExtension SDK。registry-backed SDK 返回 `distributionMode: "registry"` 和 npm、PyPI 或 Maven Central 安装命令；Swift iOS/macOS 继续返回 `distributionMode: "local_source"`。当 registry 不可用或企业环境需要 vendoring 时，agent 才使用返回的 `localSourceFallback`、GitHub clone、`vendor/` 复制、本地依赖、SwiftPM local path、Gradle module 或 PYTHONPATH 指令。返回的 `projectKey` 只能用于 Auto Capture 写入，不能替代 MCP token。普通 server 平台额外返回 `projectKeyUsage`、`configurationModes`、`preDeployChecks`、`postDeployVerification` 和 `expectedCaptureQuery`，把部署前检查和部署后真实事件验证变成结构化 contract。
...
Web 平台还会返回 `webCaptureScript.latestReleaseId`、`webCaptureScript.upgradePrompt` 和 `webCaptureScript.verificationSteps`。如果 `project_health` 返回 `captureScriptFindings`，客户 agent 应用这些步骤把固定 hash asset 或本地复制脚本改回返回的稳定 `captureScriptUrl`，并检查 service worker、Workbox、CDN、反向代理和 WebView bundle 是否缓存旧脚本。

Server 平台的 `configurationModes` 固定包含两个模式：`inline_project_key` 是推荐默认值，直接使用 `capture_setup` 返回的 public `projectKey`；`env_project_key` 是可选工程偏好，变量名固定为 `TRACEMIND_PROJECT_KEY`，只在客户仓库已有部署环境变量惯例时使用。`TRACEMIND_PRODUCT_USAGE_PROJECT_ID/KEY` 只属于 TraceMind 内部 dogfood marker，不属于客户 server 接入参数；MCP token 和 Bearer token 也不能当作 capture key。
...
```json
{
  ...
  "platform": "web",
  "eventPlatform": "web",
  "captureScriptUrl": "https://tracemind-capture.pages.dev/capture.js",
  ...
  "webCaptureScript": {
    ...
    "verificationSteps": [
      "Open the customer app and confirm window.TraceMind.status().scriptReleaseId === \"2026.08.15.2\".",
...
```json
{
  ...
  "installCommands": [
    "npm install @tracemind/server-node@<version>",
  ...
  "configurationModes": [
    ...
    },
    {
      ...
      "recommended": false,
      "variableName": "TRACEMIND_PROJECT_KEY"
    }
  ],
  "preDeployChecks": [
    "Confirm the returned initSnippet runs once during server startup before any business handler calls capture.",
    "Confirm capture setup uses the returned public projectKey only.",
    "If using env_project_key mode, confirm the deployment environment defines TRACEMIND_PROJECT_KEY.",
    "Confirm the server can egress to captureApiUrl."
  ],
...
部署后验证使用客户刚刚埋点的真实业务事件，不新增 setup probe event。若 `postDeployVerification` 查询结果为 0，agent 必须先按返回的 `failureTriage` 排查初始化、key/env、网络出口、delivery 和事件名，而不是直接判断客户服务端没有活跃。
