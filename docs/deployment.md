...
- `https://tracemind.sandbox.galaxycloud.app/service-worker.js` returns JavaScript that installs and activates at root scope, caches the anonymous page shell plus PWA/static assets, and explicitly bypasses OAuth callbacks (`/_oauth/`), dashboard/API data, MCP, capture, presence, feedback, authenticated content, and Meteor realtime routes.
- After a Service Worker update, verify the new worker is active and old page caches are removed. With a populated shell cache, complete both Google and GitHub login: callback navigation must reach the server, the popup must close, and the original page must finish login. A callback displaying the homepage is a failed check; do not infer success from provider authorization alone.
