...
| Semantic extraction + reports | `server/semantic_jobs.js`, `server/daily_reports.js`, `server/health_alerts.js`, `imports/api/semantic.js` | Periodically convert raw behavior into semantic events, maintain project health reports, and evaluate opted-in completed-hour incident/recovery emails |
...
- The selected-project console health overview reads Asia/Shanghai daily reports generated from hourly rollups instead of scanning all historical events on refresh. The dashboard subscribes to safe `tracemind_project_daily_reports` fields and uses Minimongo as the date-switch cache. A background job refreshes projects with recent completed-hour activity and updates today's draft report from completed hours compared with yesterday's same completed-hour span; when the selected date is today, an open visible Dashboard also reuses the same queued refresh path every 5 minutes and shows an automatic update status instead of a manual refresh button. Historical dates read already-materialized final reports from pub/sub. Reports store hashed actor and session sets internally and persist only public health metrics for the client. The four core health cards also receive sanitized hourly comparison series so the UI can render current-day vs previous-day mini trends without exposing internal merge keys.
- Project owners may opt in to email-only failure health alerts. After refreshing the just-completed hour, the same job compares its exact hourly report with yesterday's matching hour and emails the existing owner address only when `failure_events_increased` opens or recovers. `event_stream_stopped` remains a Dashboard/MCP health item but is not email-eligible. The server advances compact Project state only after successful delivery and keeps internal state out of publications and public project responses.
- Actor Metrics Phase 2A keeps the existing actor card on its v1 observed-actor axis: `activeUsers`, its trend, hourly sparkline, and retention stay on that legacy contract, and first-seen/retention values remain visibly labelled as legacy. When `actorMetricsV2.coverage` is `complete`, its aggregate values appear only as an expanded deterministic explanation; `partial` and `unavailable` hide every v2 count. This adds no card, API, or UI switcher.
- Today's selected-project dashboard also has a separate delayed recent-online card. It calls `tracemind.project.recentOnline` after the main cards render, scans the 30-minute window ending at the most recently completed five-minute boundary for both presence sessions and semantic events, and returns unique online users, six 5-minute buckets, region Top 3, active-duration page Top 3, and event Top 3. All card metrics and MCP recent-online results share this aligned window, excluding the unfinished interval. Bucket labels show end times. The card refreshes every 60 seconds while the Dashboard tab is visible. This path is intentionally outside daily reports so it cannot slow date switching or project setup metadata.
