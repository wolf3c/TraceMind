...
- Project owners may opt in to email-only high-severity health alerts. After refreshing the just-completed hour, the same job compares its exact hourly report with yesterday's matching hour and reuses the existing health attention rules. The server emails the existing owner address only on incident and recovery transitions, advances compact Project state only after successful delivery, and keeps internal state out of publications and public project responses.
