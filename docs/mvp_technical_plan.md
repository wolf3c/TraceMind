...
- Today's selected-project dashboard also has a separate delayed recent-online card. It calls `tracemind.project.recentOnline` after the main cards render, scans only the last 30 minutes of presence sessions and semantic events, and returns unique online users, six 5-minute buckets, region Top 3, active-duration page Top 3, and event Top 3. The card refreshes every 60 seconds while the Dashboard tab is visible. This path is intentionally outside daily reports so it cannot slow date switching or project setup metadata.
