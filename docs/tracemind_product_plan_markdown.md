...
- 用户在哪里遇到阻碍
- 哪些功能真正有价值
- 每天早上第一眼，当前项目是否正常运行、是否需要关注
- 当前在线人数复用最近 15 秒心跳且未结束或后台的会话去重，Dashboard 每 15 秒刷新；历史在线人数仍截至最近已结束的整 5 分钟，向前半小时统计，地区、活跃页面、高频事件和柱图均属于历史窗口，柱图标注区间结束时间
...
Web/PWA 的页面缓存不得拦截 Google/GitHub 登录回调；授权完成后应关闭弹窗并恢复原页面登录流程。
