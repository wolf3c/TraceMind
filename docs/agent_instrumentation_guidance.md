...
- 混合应用不新增 `hybrid` 事件平台；WebView 使用返回 snippet 里的 `data-tracemind-framework` 写入来源 metadata，原生壳层保持 `ios`、`macos` 或 `android`。Web `framework` 表示运行容器：普通页面自动为 `browser`，具体显式值优先，`hybrid` 仅在无法通过高置信度宿主信号识别 Capacitor、Electron、Tauri、Cordova 或 PWA 时保留；宿主未暴露官方信号时显式配置，不用 UA 或内部变量猜测。
