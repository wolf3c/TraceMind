...
Web script release `2026.07.19.1` 起，队列诊断会记录最近一次失败 flush 的安全 ISO 时间 `lastFailedFlushAt`，但不会保存原始请求、响应、URL 或日志。服务端仅为失败、重试或丢弃的 flush 保留 7 天诊断明细；`tracemind.query_delivery_diagnostics` 先按平台和来源过滤，再应用 5,000 条分析上限，并按小时及脱敏原因类别聚合，用后续成功上报时间计算可用的恢复耗时。MCP 不回显 `lastFailedFlushAt`；旧 runtime 没有该字段时恢复耗时样本数为 0，不做推算。成功 flush 继续只进入小时级上报健康聚合。
...
- `tracemind_capture_delivery_reports` 保留 7 天，仅用于近期上报失败、重试、丢弃等异常明细排查。`tracemind.query_delivery_diagnostics` 只返回脱敏后的小时/source/platform、reason 类别、HTTP 状态类别、队列深度、重试/丢弃计数和可用恢复耗时；不返回原始错误、请求/响应体、URL、日志、用户内容或 session/device/batch 标识。成功 flush 只写入小时级上报健康聚合；更早的上报健康以 `project_health.delivery`、小时报告和日报为准。
