...
普通后端服务不在第一版 Auto Capture 信号列表里。它们只通过 `server_node`、`server_python` 或 `server_http` 手动上报稳定业务结果，例如支付成功、账单已付、工作区创建、任务完成或同步完成。部署前必须跑 `preDeployChecks`，部署后触发真实业务路径并按 `postDeployVerification` 查询 `eventName + sourceType/server_app + sourceKey`；结果为 0 时先排查初始化、projectKey/env、egress/TLS/proxy、`/api/capture` delivery 和事件名，不直接判断客户无活跃。
...
Web 接入代码继续使用稳定的 `/capture.js` 地址，方便客户自动跟随当前部署版本并兼容已经接入的旧 snippet。生产环境由 `TRACEMIND_CAPTURE_SCRIPT_ORIGIN` 指向 Cloudflare Pages，例如 `https://tracemind-capture.pages.dev/capture.js`；脚本内部默认仍把 `/api/capture`、`/api/presence` 和 `/api/user-feedback` 发往 Galaxy API origin。Cloudflare 入口带 `ETag`、CORS 和约 60 秒短缓存；部署回滚后，客户最多等待入口缓存过期即可回到回滚版本，不需要修改 snippet。Galaxy `/capture.js` 继续作为本地开发、旧链接兼容和紧急回滚 fallback，不作为生产缓存头合规目标。内容哈希命名的脚本路径作为附加 immutable asset 保留，但默认接入代码不依赖它。
...
普通前台发送会按批次写入 Galaxy `/api/capture` 或 `/api/presence`，每批默认最多 20 条。发送失败时事件保留在队列中，使用 1 秒起步、60 秒封顶的指数退避重试。`online`、`visibilitychange`、`pagehide`、`beforeunload`、presence heartbeat 和 `window.TraceMind.flush()` 都会触发 flush。页面隐藏或卸载时，脚本会把单批控制在约 60KB 内并优先使用 `sendBeacon`。Web 脚本会在 `sourceDetails.scriptReleaseId` 上报当前采集脚本 release id；当 `/api/capture` 或 `/api/presence` 发现旧脚本仍在运行时，响应里返回 `webCaptureScriptUpdate`，支持自动更新的新脚本会用配置的脚本分发 origin 加 cache-buster 拉取最新 `/capture.js` 并让新 runtime 接管，老 runtime 停止上报。

每条 Web capture 队列记录还会把现有队列 ID 作为内部 `clientEventId` 随重试透传。服务端按 `projectId + clientEventId` 保证 Raw Behavior 只写入一次：响应丢失后的相同队列记录返回幂等成功并从客户端队列移除，而相同错误指纹的不同真实发生仍分别保存。该字段不进入 Semantic Event、Dashboard、MCP 或业务属性；旧脚本没有 `clientEventId` 时继续按原有兼容路径接收。本合同只覆盖 Web 与复用 `capture.js` 的 Hybrid WebView，不代表其他 SDK 已具备相同幂等保证。

| Runtime / Surface | Impact |
| --- | --- |
| Web | Change: capture queue retries use the existing record ID for project-scoped idempotency. |
| Hybrid | Change only in the WebView using `capture.js`; the native layer is unchanged. |
| iOS, macOS, Android, React Native | No change. |
...
- `tracemind_capture_delivery_reports` 保留 7 天，仅用于近期上报异常明细排查。`tracemind.query_delivery_diagnostics` 返回脱敏后的小时/endpoint/source/platform、恢复分类、证据质量、时长组成、归因覆盖率，以及单独命名的旧版墙钟耗时；不返回运行实例/区间标识或原始诊断内容。成功 flush 只写入小时级上报健康聚合；更早的上报健康以 `project_health.delivery`、小时报告和日报为准。
...
- `tracemind_project_hourly_reports` 和 `tracemind_project_daily_reports` 当前长期保留，是历史分析的主要数据来源。

错误上下文使用 `app_error` 进入普通行为队列，Web 自动监听 `window.error` 和 `unhandledrejection`，手动场景使用 `window.TraceMind.captureError(...)`。该事件只保存错误类型、脱敏短消息预览、消息/stack/top-frame/cause 指纹、handled/fatal、component/release、operation/feature、correlation/request id、HTTP 状态和 path/screen 等摘要字段；原始 stack trace、raw log、raw message、源码、请求/响应 body、headers、cookies、authorization、输入值、prompt、raw user content、secret、截图、录屏、crash dump 和 session replay 都不进入 v1。
...
- `sourceType` / `sourceKey` / `sourceLabel` / `sourceDetails`: 从 SDK payload 和请求头归一化得到的来源字段。Web 的 `sourceKey` 优先使用请求 `Origin`，其次使用请求 `Referer`，最后才回退到 SDK payload URL，并白名单保留 `sourceDetails.scriptReleaseId` 和 `framework`。新 Web 事件的 `framework` 表示初始化时识别一次的运行容器：有效显式具体值优先；`hybrid` 可升级为按固定优先级识别的 `capacitor`、`electron`、`tauri`、`cordova` 或 `pwa`，否则保留 `hybrid`；无配置且无可靠信号时为 `browser`。检测访问异常只回落，不阻断采集；不使用 UA、Tauri 内部变量或自定义 bridge 猜测。iOS 使用 bundle id；Android 使用 package name；React Native 复用原生来源并在 `deviceInfo.framework` 或 `sourceDetails.framework` 标记 `react_native`；混合应用不新增事件平台，WebView 保持 Web 来源，原生壳层保持对应 Native 来源；小程序使用 `sourceType: "mini_program"`，`sourceKey` 优先使用 appId，没有 appId 时使用配置的 sourceKey，只白名单保留 `sourceDetails.provider`、`sdkVersion` 和 `sdkContentHash`；浏览器插件使用 `sourceType: "browser_extension"`，`sourceKey` 优先使用 extension id，只白名单保留 `sourceDetails.browser`、`manifestVersion`、`runtimeContext`、`sdkVersion` 和 `sdkContentHash`；MCP server 使用配置的 server/package 名作为 `sourceKey`；普通后端服务使用 `sourceType: "server_app"` 和配置的服务名；Agent Skill 使用稳定 Skill 名或宿主 runtime skill id。SDK 升级只通过白名单的 `sdkVersion` / `sdkContentHash` 判断；Web 脚本升级只通过 `scriptReleaseId` 判断，不透传任意 source details。
