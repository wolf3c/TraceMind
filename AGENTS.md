...
Use the global Error Ledger rules, with this format:
...
- `2026-05-27`: X monitoring was attempted through Chrome DevTools MCP instead of the `@Chrome` plugin. Root cause: page-opening capability was confused with the required logged-in browser context for account-gated social checks. Future rule: for X/Twitter, Gmail web, 小红书, 即刻, or any platform where cookies/login state determine visibility, use `@Chrome` directly; reserve DevTools/browser inspection for unauthenticated or local-page checks only.
...
Pull requests should include a short summary, testing performed, linked issue or task when available, and screenshots for visible UI changes.
