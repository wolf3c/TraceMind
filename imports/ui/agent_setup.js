...
If \`tracemind.project_health\` returns \`captureScriptFindings\`, call \`tracemind.capture_setup({ platform: "web" })\`, replace fixed \`capture.<hash>.js\` or self-hosted Web scripts with stable \`/capture.js\`, check service worker/Workbox/CDN/reverse proxy/WebView bundle caches, verify \`window.TraceMind.status().scriptReleaseId\`, trigger a real behavior, and re-check \`project_health\`.
...
如果 \`tracemind.project_health\` 返回 \`captureScriptFindings\`，调用 \`tracemind.capture_setup({ platform: "web" })\`，把固定 \`capture.<hash>.js\` 或自托管 Web 脚本改回稳定 \`/capture.js\`，检查 service worker/Workbox/CDN/反向代理/WebView bundle 缓存，验证 \`window.TraceMind.status().scriptReleaseId\`，触发一次真实行为后再次调用 \`project_health\`。
