...
After installation, you can ask the agent to review product health, real-time online status, last 24 hours performance, traffic sources, and recent delivery failures. Operations questions should use Dashboard-aligned metrics first: call \`tracemind.project_info\` and read \`availableCapabilities\`, then call \`tracemind.project_health\` and \`tracemind.recent_online\` before any instrumentation setup. Use \`availableCapabilities.currentOnline.tool\` for current online users and last 30 minutes activity; use \`availableCapabilities.projectHealth.tool\` for product health, today health, delivery health, attention items, and change verification; use \`availableCapabilities.deliveryDiagnostics.tool\` for recent privacy-safe delivery root-cause drilldown. Then use \`tracemind.summary\` and \`tracemind.query_events\` for non-natural-day windows and evidence drilldown.
After installation, call \`tracemind.project_health\` first for a daily health check and \`tracemind.recent_online\` for real-time online status. For delivery failures, retries, drops, queue depth, or recovery within 7 days, call \`tracemind.query_delivery_diagnostics\`; use endpoint, recovery classification, evidence quality, duration composition, and attribution coverage. Treat \`recoveryDurationMs\` as episode-attributed and \`legacyElapsedDurationMs\` only as unattributed wall-clock time, never foreground wait. Runtime/episode IDs and raw diagnostic content are omitted. Use \`lifecycleState\` and \`connectivityState\` filters for evidence-backed event analysis and report missing-context coverage. Then use \`tracemind.summary\` and \`tracemind.query_events\` for feature usage analysis, anomaly or drop investigation, and behavior evidence. Treat \`tracemind.summary\` totals as sample-derived according to \`summarySample\`, not as full-day totals or current online users. Use \`tracemind.query_raw_behaviors\` only when semantic evidence is insufficient.
If the current active tool list does not show \`tracemind.project_health\`, \`tracemind.query_delivery_diagnostics\`, \`tracemind.recent_online\`, \`tracemind.query_raw_behaviors\`, or \`tracemind.submit_feedback\`, do not conclude the tools are unavailable. First read MCP \`tools/list\` or retry discovery with the exact tool name; if they are still missing, refresh the connector/session/MCP config/token and call \`tracemind.project_info\` again to confirm the project binding and \`availableCapabilities\`. Do not compensate for missing current online or project health capabilities by increasing \`tracemind.summary.limit\`.
If \`tracemind.project_health\` returns \`captureScriptFindings\`, call \`tracemind.capture_setup({ platform: "web" })\`, replace fixed \`capture.<hash>.js\` or self-hosted Web scripts with the returned stable \`captureScriptUrl\`, check service worker/Workbox/CDN/reverse proxy/WebView bundle caches, verify \`window.TraceMind.status().scriptReleaseId\`, trigger a real behavior, and re-check \`project_health\`.
...
安装完成后，可直接让 agent 查看产品健康、实时在线、过去 24 小时表现、流量来源和近期上报失败。运营查询优先使用 Dashboard 同源口径：先调用 \`tracemind.project_info\` 并读取 \`availableCapabilities\`，再调用 \`tracemind.project_health\` 和 \`tracemind.recent_online\`，不要先进入埋点安装。\`availableCapabilities.currentOnline.tool\` 是当前在线和近 30 分钟态势入口；\`availableCapabilities.projectHealth.tool\` 是产品健康、今日健康、上报健康和变更验证入口；\`availableCapabilities.deliveryDiagnostics.tool\` 是近期上报根因的脱敏下钻入口。再用 \`tracemind.summary\` 和 \`tracemind.query_events\` 查询非自然日时间窗和证据下钻。
安装完成后，优先调用 \`tracemind.project_health\` 做今日健康检查，并调用 \`tracemind.recent_online\` 查看实时在线态势。遇到 7 天内的上报失败、重试、丢弃、队列深度或恢复问题，调用 \`tracemind.query_delivery_diagnostics\`，按 endpoint、恢复分类、证据质量、时长组成和归因覆盖率分析。\`recoveryDurationMs\` 是区间证据归因，\`legacyElapsedDurationMs\` 只是旧版未归因墙钟时间，不能解释为前台等待；输出不含运行实例/区间标识或原始诊断内容。事件分析可用 \`lifecycleState\` / \`connectivityState\`，并说明上下文缺失率。然后使用 \`tracemind.summary\` 和 \`tracemind.query_events\` 做功能使用分析、异常或下降原因分析和行为证据下钻。必须按 \`summarySample\` 把 \`tracemind.summary\` 的 totals 视为样本口径。只有语义证据不足时，才调用 \`tracemind.query_raw_behaviors\`。
如果当前 active tool list 看不到 \`tracemind.project_health\`、\`tracemind.query_delivery_diagnostics\`、\`tracemind.recent_online\`、\`tracemind.query_raw_behaviors\` 或 \`tracemind.submit_feedback\`，不要直接判断工具不可用。先读取 MCP \`tools/list\` 或按精确工具名重新 discovery；如果仍缺失，刷新 connector/session/MCP 配置/token，再调用 \`tracemind.project_info\` 复核项目绑定和 \`availableCapabilities\`。不要通过增大 \`tracemind.summary.limit\` 来代偿缺失的 current online 或 project health 能力。
如果 \`tracemind.project_health\` 返回 \`captureScriptFindings\`，调用 \`tracemind.capture_setup({ platform: "web" })\`，把固定 \`capture.<hash>.js\` 或自托管 Web 脚本改回返回的稳定 \`captureScriptUrl\`，检查 service worker/Workbox/CDN/反向代理/WebView bundle 缓存，验证 \`window.TraceMind.status().scriptReleaseId\`，触发一次真实行为后再次调用 \`project_health\`。
