...
After installation, call \`tracemind.project_health\` first for a daily health check and \`tracemind.recent_online\` for real-time online status. Then use \`tracemind.summary\` and \`tracemind.query_events\` for feature usage analysis or anomaly or drop investigation. Treat \`tracemind.summary\` totals as sample-derived according to \`summarySample\`, not as full-day totals. Use \`tracemind.query_raw_behaviors\` only when semantic evidence is insufficient.
If the current active tool list does not show \`tracemind.project_health\`, \`tracemind.query_raw_behaviors\`, or \`tracemind.submit_feedback\`, do not conclude the tools are unavailable. First read MCP \`tools/list\` or retry discovery with the exact tool name; if they are still missing, refresh the connector/session/MCP config/token and call \`tracemind.project_info\` again. Do not compensate for missing reporting tools by increasing \`tracemind.summary.limit\`.
...
安装完成后，优先调用 \`tracemind.project_health\` 做今日健康检查，并调用 \`tracemind.recent_online\` 查看实时在线态势。然后使用 \`tracemind.summary\` 和 \`tracemind.query_events\` 做功能使用分析、异常或下降原因分析。必须按 \`summarySample\` 把 \`tracemind.summary\` 的 totals 视为样本口径，不能当作全天总数。只有语义证据不足时，才调用 \`tracemind.query_raw_behaviors\` 复核原始行为。
如果当前 active tool list 看不到 \`tracemind.project_health\`、\`tracemind.query_raw_behaviors\` 或 \`tracemind.submit_feedback\`，不要直接判断工具不可用。先读取 MCP \`tools/list\` 或按精确工具名重新 discovery；如果仍缺失，刷新 connector/session/MCP 配置/token，再调用 \`tracemind.project_info\` 复核项目绑定。不要通过增大 \`tracemind.summary.limit\` 来代偿缺失的 reporting tools。
