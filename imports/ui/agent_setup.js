...
After MCP is configured, Call \`tracemind.capture_setup\` to retrieve the Web Auto Capture script for the current project. For iOS, macOS, Android, React Native, hybrid, mini program, browser extension, MCP server, Agent Skill, or server application projects, pass the matching \`platform\` argument and use the returned one-line initialization code plus setup checks. Use registry install commands when \`distributionMode: "registry"\`; if the setup returns \`distributionMode: "local_source"\` or \`localSourceFallback\` is explicitly needed, use its GitHub clone, \`vendor/\` copy, local dependency, SwiftPM local path, Gradle module, or PYTHONPATH instructions exactly as returned. Use the returned public projectKey only for capture writes; never use the MCP token in frontend code.
