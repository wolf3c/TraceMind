...
After installation, call \`tracemind.project_health\` first for a daily health check and \`tracemind.recent_online\` for real-time online status. For delivery failures, retries, drops, queue depth, or recovery duration within 7 days, call \`tracemind.query_delivery_diagnostics\`; it returns only privacy-safe hourly source/platform, reason class, HTTP status class, queue depth, retry/drop counts, and available recovery duration, never raw errors, request/response bodies, URLs, logs, user content, or session/device/batch identifiers. Then use \`tracemind.summary\` and \`tracemind.query_events\` for feature usage analysis or anomaly or drop investigation. Treat \`tracemind.summary\` totals as sample-derived according to \`summarySample\`, not as full-day totals or current online users. Use \`tracemind.query_raw_behaviors\` only when semantic evidence is insufficient.
...
安装完成后，优先调用 \`tracemind.project_health\` 做今日健康检查，并调用 \`tracemind.recent_online\` 查看实时在线态势。遇到 7 天内的上报失败、重试、丢弃、队列深度或恢复耗时问题，调用 \`tracemind.query_delivery_diagnostics\`；它只返回脱敏后的小时/source/platform、reason 分类、HTTP 状态类别、队列深度、重试/丢弃计数和可用恢复耗时，不返回原始错误、请求/响应体、URL、日志、用户内容或 session/device/batch 标识。然后使用 \`tracemind.summary\` 和 \`tracemind.query_events\` 做功能使用分析、异常或下降原因分析。必须按 \`summarySample\` 把 \`tracemind.summary\` 的 totals 视为样本口径，不能当作全天总数或当前在线人数。只有语义证据不足时，才调用 \`tracemind.query_raw_behaviors\` 复核原始行为。
