...
After MCP is configured, Call \`tracemind.capture_setup\` to retrieve the Web Auto Capture script for the current project. For iOS, macOS, Android, React Native, hybrid, mini program, browser extension, MCP server, Agent Skill, or server application projects, pass the matching \`platform\` argument and use the returned one-line initialization code plus setup checks. If the setup returns \`distributionMode: "local_source"\`, use its GitHub clone, \`vendor/\` copy, local dependency, SwiftPM local path, Gradle module, or PYTHONPATH instructions exactly as returned instead of assuming a registry package exists. Use the returned public projectKey only for capture writes; never use the MCP token in frontend code.
...
MCP 配好后，通过 \`tracemind.capture_setup\` 获取 Web Auto Capture 接入脚本。对于 iOS、macOS、Android、React Native、混合应用、小程序、浏览器插件、MCP server、Agent Skill 或 server application 项目，传入对应 \`platform\` 并使用返回的一行初始化代码和接入检查。如果返回 \`distributionMode: "local_source"\`，按返回的 GitHub clone、\`vendor/\` 复制、本地依赖、SwiftPM local path、Gradle module 或 PYTHONPATH 指令执行，不要假设 SDK 已发布到 registry。只把返回的公开 projectKey 用于采集写入，不要把 MCP token 写进前端代码。
