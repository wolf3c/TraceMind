...
After installation, you can ask the agent to review product health, real-time online status, last 24 hours performance, and traffic sources. Operations questions should use Dashboard-aligned metrics first: call \`tracemind.project_info\` and read \`availableCapabilities\`, then call \`tracemind.project_health\` and \`tracemind.recent_online\` before any instrumentation setup. Use \`availableCapabilities.currentOnline.tool\` for current online users and last 30 minutes activity; use \`availableCapabilities.projectHealth.tool\` for product health, today health, delivery health, attention items, and change verification. Then use \`tracemind.summary\` and \`tracemind.query_events\` for non-natural-day windows and evidence drilldown.
...
安装完成后，可直接让 agent 查看产品健康、实时在线、过去 24 小时表现和流量来源。运营查询优先使用 Dashboard 同源口径：先调用 \`tracemind.project_info\` 并读取 \`availableCapabilities\`，再调用 \`tracemind.project_health\` 和 \`tracemind.recent_online\`，不要先进入埋点安装。\`availableCapabilities.currentOnline.tool\` 是当前在线和近 30 分钟态势入口；\`availableCapabilities.projectHealth.tool\` 是产品健康、今日健康、上报健康和变更验证入口。再用 \`tracemind.summary\` 和 \`tracemind.query_events\` 查询非自然日时间窗和证据下钻。
安装完成后，优先调用 \`tracemind.project_health\` 做今日健康检查，并调用 \`tracemind.recent_online\` 查看实时在线态势。然后使用 \`tracemind.summary\` 和 \`tracemind.query_events\` 做功能使用分析、异常或下降原因分析。必须按 \`summarySample\` 把 \`tracemind.summary\` 的 totals 视为样本口径，不能当作全天总数或当前在线人数。只有语义证据不足时，才调用 \`tracemind.query_raw_behaviors\` 复核原始行为。
如果当前 active tool list 看不到 \`tracemind.project_health\`、\`tracemind.recent_online\`、\`tracemind.query_raw_behaviors\` 或 \`tracemind.submit_feedback\`，不要直接判断工具不可用。先读取 MCP \`tools/list\` 或按精确工具名重新 discovery；如果仍缺失，刷新 connector/session/MCP 配置/token，再调用 \`tracemind.project_info\` 复核项目绑定和 \`availableCapabilities\`。不要通过增大 \`tracemind.summary.limit\` 来代偿缺失的 current online 或 project health 能力。
