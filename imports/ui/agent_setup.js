...
After installation, call \`tracemind.project_health\` first for a daily health check. Then use \`tracemind.summary\` and \`tracemind.query_events\` for feature usage analysis or anomaly or drop investigation. Use \`tracemind.query_raw_behaviors\` only when semantic evidence is insufficient.
...
安装完成后，优先调用 \`tracemind.project_health\` 做今日健康检查。然后使用 \`tracemind.summary\` 和 \`tracemind.query_events\` 做功能使用分析、异常或下降原因分析。只有语义证据不足时，才调用 \`tracemind.query_raw_behaviors\` 复核原始行为。
