...
* **Auth Surfaces:** Put Google and GitHub login before the email fallback when OAuth is configured. Provider buttons should stay inside the TraceMind visual system: Porcelain backgrounds, Sage borders, Graphite text, Signal Teal focus/hover rings, and compact monochrome marks. Do not use neon provider-colored blocks or add another nested card around the login choices.
