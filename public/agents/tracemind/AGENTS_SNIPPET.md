...
4. Verify TraceMind Auto Capture before manual custom events by calling `tracemind.capture_setup`; Web uses the returned `captureSnippet`, while iOS, Android, React Native, MCP Node, MCP Python, and Agent Skill targets pass the matching `platform` (`ios`, `android`, `react_native`, `mcp_node`, `mcp_python`, or `agent_skill`) and follow the returned `installCommands`, `filesToEdit`, `initLocation`, `idempotencyChecks`, and one-line `initSnippet`. The public project key is only for capture writes.
