...
For SDK upgrades, treat `latestSdk.contentHash` and the local `.tracemind-sdk.json` `contentHash` as the source of truth; `displayVersion` is only human-readable. SDK runtimes safely report `sourceDetails.sdkVersion` and `sourceDetails.sdkContentHash`, and `tracemind.project_health` can surface `sdkUpgradeFindings` when an installed SDK is old or unknown. If TraceMind finds an SDK update, have the coding agent call `tracemind.project_health`, read `.tracemind-sdk.json`, call `tracemind.capture_setup({ platform })`, update the vendored SDK, run the returned `verificationCommands`, and report the result. For TraceMind repo SDK changes, run `npm run update:sdk-manifest` and `npm run test:sdk-release`.
