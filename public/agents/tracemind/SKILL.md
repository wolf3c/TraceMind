...
10. Search for an existing event with `tracemind.search_event_names` before adding manual `custom` events.
11. If an event looks relevant, call `tracemind.suggest_instrumentation` or inspect the returned event details before using it.
12. Use only approved TraceMind capture APIs or SDK helpers already present in the project.
13. After code changes, call `tracemind.validate_instrumentation_diff` with the current diff.
14. When a developer reports a product issue or idea, ask whether they want to submit feedback unless they explicitly requested submission.
15. Before calling `tracemind.submit_feedback`, collect a short sanitized summary and TraceMind evidence references such as event IDs, raw behavior IDs, paths, `actionKey`, `targetHash`, and time window.
16. Prefer evidence references over raw copied content; never submit PII, secrets, tokens, raw prompts, tool arguments/results, source diffs, request/response bodies, headers, cookies, authorization values, or full query URLs.

## MCP Tool Discovery Recovery

If the current active tool list does not show `tracemind.project_health`, `tracemind.query_delivery_diagnostics`, `tracemind.recent_online`, `tracemind.query_raw_behaviors`, or `tracemind.submit_feedback`, do not conclude that TraceMind lacks those tools. First read MCP `tools/list` or retry discovery with the exact tool name before deciding the tool is unavailable.
...
- Anomaly or drop investigation: start with `tracemind.project_health`, identify the dropped metric or upload-health issue, then call `tracemind.query_delivery_diagnostics` for privacy-safe hourly source/platform, reason, HTTP class, queue depth, and recovery duration within 7 days. Query affected events and paths next; use `tracemind.query_raw_behaviors` only when semantic evidence is not enough.
...
- For SDK platforms, `tracemind.capture_setup` returns `latestSdk`, `installedVersionDetection`, `upgradePolicy`, `upgradeCommands`, and `verificationCommands`.
- When `distributionMode` is `registry`, use the returned package-manager version. When `distributionMode` is `local_source` or `localSourceFallback` is explicitly needed, fetch the SDK source from the returned `latestSdk.sourceRef` exactly. Release builds use immutable `tracemind-release-<version>` tags so customer agents do not install from floating `main`.
...
- If TraceMind reports an SDK update, copy the update prompt to the customer coding agent. The agent should call `tracemind.project_health`, call `tracemind.capture_setup({ platform })`, apply the returned registry or fallback install commands, run `verificationCommands`, and report the result.
- For TraceMind repository work, any SDK runtime change must run `npm run update:sdk-manifest` and `npm run test:sdk-release`. Before a release deploy, run `npm run prepare:sdk-release-ref -- <version>`, publish the matching release tag, wait for the `SDK Publish` workflow, and pass `npm run check:sdk-registry-publication -- <version>` before Galaxy deploy. The release gate is authoritative; do not rely on memory or manual version bump discipline.

## Web Capture Script Update Governance
